...
maps.n["<leader>q"] = {
  function() require("utils").confirm_quit() end,
  desc = "Quit",
}
maps.n["<Tab>"] = {
  "<Tab>",
...
maps.n["<leader>c"] = { -- Close window and buffer at the same time.
  function() require("utils.buffer").wipe() end,
  desc = "Wipe buffer",
}
maps.n["<leader>C"] = { -- Close buffer keeping the window.
  function() require("utils.buffer").close() end,
  desc = "Close buffer",
}
-- Close buffer keeping the window → Without confirmation.
-- maps.n["<leader>X"] = {
--   function() require("utils.buffer").close(0, true) end,
...
maps.n["K"] = {
  function()
    require("utils.buffer").nav(vim.v.count > 0 and vim.v.count or 1)
  end,
  desc = "Next buffer",
}
maps.n["J"] = {
...
maps.n[">b"] = {
  function()
    require("utils.buffer").move(vim.v.count > 0 and vim.v.count or 1)
  end,
...
maps.n["<leader>bc"] = {
  function() require("utils.buffer").close_all(true) end,
  desc = "Close all buffers except current",
}
maps.n["<leader>bC"] = {
  function() require("utils.buffer").close_all() end,
...
maps.n["<leader>bl"] = {
  function() require("utils.buffer").close_left() end,
  desc = "Close all buffers to the left",
}
maps.n["<leader>br"] = {
  function() require("utils.buffer").close_right() end,
  desc = "Close all buffers to the right",
}
maps.n["<leader>bs"] = icons.bs
maps.n["<leader>bse"] = {
  function() require("utils.buffer").sort "extension" end,
  desc = "Sort by extension (buffers)",
}
maps.n["<leader>bsr"] = {
  function() require("utils.buffer").sort "unique_path" end,
  desc = "Sort by relative path (buffers)",
}
maps.n["<leader>bsp"] = {
  function() require("utils.buffer").sort "full_path" end,
  desc = "Sort by full path (buffers)",
}
maps.n["<leader>bsi"] = {
  function() require("utils.buffer").sort "bufnr" end,
  desc = "Sort by buffer number (buffers)",
}
maps.n["<leader>bsm"] = {
  function() require("utils.buffer").sort "modified" end,
